...
{  
  uint16_t refcolumn = 0;
  /* 判断显示位置不能超出液晶的边界 */
  while ((refcolumn < LCD_PIXEL_WIDTH) && ((*ptr != 0) & (((refcolumn +  WIDTH_CH_CHAR) & 0xFFFF) >= WIDTH_CH_CHAR)))
  {
	/* 使用LCD显示一个字符 */
	if ( * ptr <= 126 )	           	//英文字符
	{	
...
//WIDTH_CH_CHAR		              32	    //中文字符宽度 
//#define      HEIGHT_CH_CHAR
...
static void LCD_DispChar_CH (uint16_t Xpos, uint16_t Ypos, uint16_t Text)
{
  ...
  for(i = 0; i < height; i++)//遍历字体高度绘点
  {
    ...
    case 4:
      default:
      line =  (pchar[0]<< 24) | (pchar[1]<< 16) |(pchar[2]<<8)|pchar[3]; //提取字体宽度大于16小于24的字符的像素值      
    } 
