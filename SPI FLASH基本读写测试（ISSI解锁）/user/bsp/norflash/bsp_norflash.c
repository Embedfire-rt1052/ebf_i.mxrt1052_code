...
#define NOR_CMD_LUT_SEQ_IDX_WRITEFUNREG              		6
#define NOR_CMD_LUT_SEQ_IDX_READFUNREG               		7
...
static void FlexSPI_NorFlash_WriteBp(void);
/*******************************************************************************
 * Variables
 ******************************************************************************/
...
//                            kFLEXSPI_Command_STOP, kFLEXSPI_1PAD, 0),  
        ...
        /* 写功能寄存器，Enable Quad mode */
        [4 * NOR_CMD_LUT_SEQ_IDX_WRITEFUNREG] =
            FLEXSPI_LUT_SEQ(kFLEXSPI_Command_SDR, kFLEXSPI_1PAD, W25Q_WriteFunReg, 
                            kFLEXSPI_Command_WRITE_SDR, kFLEXSPI_1PAD, 0x04),

        /* 读功能寄存器，Read fun register */
        [4 * NOR_CMD_LUT_SEQ_IDX_READFUNREG] =
            FLEXSPI_LUT_SEQ(kFLEXSPI_Command_SDR, kFLEXSPI_1PAD, W25Q_ReadFunReg, 
                            kFLEXSPI_Command_READ_SDR, kFLEXSPI_1PAD, 0x04),
...
void FlexSPI_NorFlash_Init(void)
{
  ...
  FlexSPI_NorFlash_Get_JedecDevice_ID(FLEXSPI, &JedecDeviceID);
	FlexSPI_NorFlash_WriteBp();
	/* ISSI厂商的FLASH需要使能Quad模式 */
	if(JedecDeviceID == FLASH_ISSI_JEDECDEVICE_ID)
	{
...
* @retval FlexSPI传输返回的状态值，正常为0
*/
status_t FlexSPI_NorFlash_Read_Funtion_Register(FLEXSPI_Type *base, uint8_t *SR_value)
{
    ...
    flashXfer.cmdType = kFLEXSPI_Read;
    flashXfer.SeqNumber = 1;
    flashXfer.seqIndex = NOR_CMD_LUT_SEQ_IDX_READFUNREG;
    flashXfer.data = &readValue;
    flashXfer.dataSize = 1;
...
* @retval FlexSPI传输返回的状态值，正常为0
*/
status_t FlexSPI_NorFlash_Write_Funtion_Register(FLEXSPI_Type *base, uint8_t *config)
{
		flexspi_transfer_t flashXfer;
    status_t status;
    ...
    flashXfer.cmdType = kFLEXSPI_Write;
    flashXfer.SeqNumber = 1;
    flashXfer.seqIndex = NOR_CMD_LUT_SEQ_IDX_WRITEFUNREG;
		flashXfer.data = (uint32_t *)config;
    flashXfer.dataSize = 1;
...
void FlexSPI_NorFlash_Test(void)
{
	uint8_t SR_value;
	status_t status;
	
	/* 读取原寄存器的内容 */
	status = FlexSPI_NorFlash_Read_Status_Register(FLEXSPI, &SR_value);
	PRINTF("SR_value:      0x%x \r\n",SR_value);  
	if (status != kStatus_Success)
	{
	...
	/* 写入该值 */
	status = FlexSPI_NorFlash_Write_Status_Register(FLEXSPI, &SR_value);
	//FlexSPI_NorFlash_Enable_Quad_Mode(FLEXSPI);
		/* 读取原寄存器的内容 */
	status = FlexSPI_NorFlash_Read_Status_Register(FLEXSPI, &SR_value);
	PRINTF("SR_value:      0x%x \r\n",SR_value); 
	SR_value=0;
		/* 读取原寄存器的内容 */
	status = FlexSPI_NorFlash_Read_Funtion_Register(FLEXSPI, &SR_value);
	PRINTF("Fun_value:      0x%x \r\n",SR_value);  

//	SR_value |=0x02;
//		/* 读取原寄存器的内容 */
//	status = FlexSPI_NorFlash_Write_Funtion_Register(FLEXSPI, &SR_value);
//	PRINTF("Fun_value:      0x%x \r\n",SR_value);  
//	if (status != kStatus_Success)
//	{
...
void FlexSPI_NorFlash_WriteBp(void)
{
	uint8_t SR_value;
	status_t status;
