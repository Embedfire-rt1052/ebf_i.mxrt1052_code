...
/* 显存 */
AT_NONCACHEABLE_SECTION_ALIGN( uint32_t s_frameBuffer[2][LCD_PIXEL_HEIGHT][LCD_PIXEL_WIDTH], FRAME_BUFFER_ALIGN);
...
{
  CurrentFrameBuffer = (uint32_t)s_frameBuffer[index];
}
...
{
  /* 设置ELCDIF的下一个缓冲区地址 */
  ELCDIF_SetNextBufferAddr(APP_ELCDIF, (uint32_t)s_frameBuffer[index]);
