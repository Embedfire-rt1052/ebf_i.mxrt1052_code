/*
 * Copyright 2016-2018 NXP
 * All rights reserved.
 *
 * SPDX-License-Identifier: BSD-3-Clause
 */

#include "sai.h"
#include "recordPlayback.h"
#include "ff.h"
/*******************************************************************************
 * Definitions
 ******************************************************************************/

/*******************************************************************************
 * Prototypes
 ******************************************************************************/

/*******************************************************************************
 * Variables
 ******************************************************************************/
extern sai_edma_handle_t txHandle;
extern sai_edma_handle_t rxHandle;
// extern sai_transfer_format_t format;
uint8_t read_and_write_Buff[SIZE * BUFFER_NUM];

extern volatile bool istxFinished;
extern volatile bool isrxFinished;

volatile uint32_t beginCount;   //总的循环次数
volatile uint32_t sendCount;    //发送个数
volatile uint32_t receiveCount; //接收计数

volatile uint32_t fullBlock = 0;           //满的缓冲区个数
volatile uint32_t emptyBlock = BUFFER_NUM; //空缓冲区个数

/*
*函数功能：录音回放
*说明：录音的同时播放录音，参数time_s设定的时间后自动停止。
*参数 base，使用的I2S基址。time_s，录音时间，单位为秒。
*/
void RecordPlayback(I2S_Type *base, uint32_t time_s)
{
    sai_transfer_t xfer = {0};
    uint32_t playbackCount = 0, recordCount = 0;
    uint32_t txindex = 0, rxindex = 0;

    /* First clear the buffer */
    memset(read_and_write_Buff, 0, SIZE * BUFFER_NUM);
    istxFinished = false;
    isrxFinished = false;
    sendCount = 0;
    // receiveCount = 0;

    /* Reset SAI internal logic */
    SAI_TxSoftwareReset(base, kSAI_ResetTypeSoftware);
    SAI_RxSoftwareReset(base, kSAI_ResetTypeSoftware);

    /* Compute the begin count */
    beginCount = time_s * SAMPLE_RATE * 4u / SIZE;

    xfer.dataSize = SIZE;
    /* Wait for playback finished */
    while ((recordCount < beginCount) || (playbackCount < beginCount))
    {
        if ((emptyBlock > 0) && (recordCount < beginCount))
        {
            xfer.data = read_and_write_Buff + rxindex * SIZE;
            if (SAI_TransferReceiveEDMA(base, &rxHandle, &xfer) == kStatus_Success)
            {
                rxindex = (rxindex + 1U) % BUFFER_NUM;
                emptyBlock--;
                recordCount++;
            }
        }

        if ((fullBlock > 0) && (playbackCount < beginCount))
        {
            xfer.data = read_and_write_Buff + txindex * SIZE;
            if (SAI_TransferSendEDMA(base, &txHandle, &xfer) == kStatus_Success)
            {
                txindex = (txindex + 1U) % BUFFER_NUM;
                fullBlock--;
                playbackCount++;
            }
        }
    }

    /* Wait for record and playback finished */
    while ((istxFinished != true) || (isrxFinished != true))
    {
    }
}

extern FIL g_fileObject;
/*
*函数功能：录音，并且把数据保存到SD卡中
*说明：执行指定时间长度的录音，录音完成后自动退出函数。
*参数 base，使用的I2S基址。time_s，录音时间，单位为秒。
*/
void RecordSDCard(I2S_Type *base, uint32_t time_s, const char* FR_NO_PATH)
{
    uint32_t i = 0;
    uint32_t bytesWritten = 0; //保存文件实际写数据量
    uint32_t txindex = 0;      //当前发送的缓冲区
    uint32_t rxindex = 0;      //当前接收的缓冲区，程序中把read_and_write_Buff缓冲区分为BUFFER_NUM个缓冲区，
    uint8_t header[44] = {0};                                 //文件头
    uint32_t fileSize = time_s * SAMPLE_RATE * 2U * 2U + 44U; //文件大小
    FRESULT error;                                            //文件打开、关闭执行结果
    sai_transfer_t xfer = {0};                                // SAI 数据传输结构体，用于SAI的数据发送与接收

    /* 复位状态标记 */
    isrxFinished = false;
    receiveCount = 0;
    istxFinished = false;
    sendCount = 0;
    // sdcard = true;

    PRINTF("\r\nBegin to record......\r\n");
    PRINTF("\r\nFile path is record/music1.wav\r\n");
    error = f_open(&g_fileObject, FR_NO_PATH, (FA_WRITE | FA_READ | FA_CREATE_ALWAYS));
    if (error)
    {
        if (error == FR_EXIST)
        {
            PRINTF("File exists.\r\n");
        }
        else
        {
            PRINTF("Open file failed.\r\n");
            return;
        }
    }

    /* Write the data into the sdcard */
    wav_header(header, SAMPLE_RATE, 16, 2, fileSize);

    /* Write the wav header */
    error = f_write(&g_fileObject, (void *)header, 44U, (UINT *)&bytesWritten);
    if ((error) || (bytesWritten != 44))
    {
        PRINTF("Write file failed. \r\n");
    }

    /* Reset SAI internal logic */
    SAI_TxSoftwareReset(base, kSAI_ResetTypeSoftware);
    SAI_RxSoftwareReset(base, kSAI_ResetTypeSoftware);

    /* Start to record */
    beginCount = time_s * SAMPLE_RATE * 2U * 2U / BUFFER_SIZE;

    /* Start record first */
    memset(read_and_write_Buff, 0, BUFFER_SIZE * BUFFER_NUM);
    xfer.dataSize = BUFFER_SIZE;
    for (i = 0; i < BUFFER_NUM; i++)
    {
        xfer.data = read_and_write_Buff + i * BUFFER_SIZE;
        SAI_TransferReceiveEDMA(base, &rxHandle, &xfer);
    }

    emptyBlock = 0;
    while ((isrxFinished != true) || (fullBlock != 0))
    {
        if (fullBlock > 0)
        {
            f_write(&g_fileObject, read_and_write_Buff + txindex * BUFFER_SIZE, BUFFER_SIZE, (UINT *)&bytesWritten);
            txindex = (txindex + 1U) % BUFFER_NUM;
            fullBlock--;
            emptyBlock++;
        }

        if ((emptyBlock > 0) && (isrxFinished == false))
        {
            xfer.data = read_and_write_Buff + rxindex * BUFFER_SIZE;
            rxindex = (rxindex + 1U) % BUFFER_NUM;
            SAI_TransferReceiveEDMA(base, &rxHandle, &xfer);
            emptyBlock--;
        }
    }
    f_close(&g_fileObject);
    PRINTF("\r\nRecord is finished!\r\n");
}



/*
*函数功能：初始化WAV格式文件的文件头
*说明：根据函数参数设置WAV格式音乐的文件头。
*参数 ：
*header, 文件头数据地址。
*sampleRate, WAV音乐的采样率。
*bitsPerFrame，数据宽度。
*channels， 通道数
*fileSize，文件大小
*/
void wav_header(uint8_t *header, uint32_t sampleRate, uint32_t bitsPerFrame, uint8_t channels, uint32_t fileSize)
{
    uint32_t totalDataLen = fileSize - 8U;
    uint32_t audioDataLen = fileSize - 44U;
    uint32_t byteRate = sampleRate * (bitsPerFrame / 8U) * channels;
    header[0] = 'R';
    header[1] = 'I';
    header[2] = 'F';
    header[3] = 'F';
    header[4] = (totalDataLen & 0xff); /* file-size (equals file-size - 8) */
    header[5] = ((totalDataLen >> 8U) & 0xff);
    header[6] = ((totalDataLen >> 16U) & 0xff);
    header[7] = ((totalDataLen >> 24U) & 0xff);
    header[8] = 'W'; /* Mark it as type "WAVE" */
    header[9] = 'A';
    header[10] = 'V';
    header[11] = 'E';
    header[12] = 'f'; /* Mark the format section 'fmt ' chunk */
    header[13] = 'm';
    header[14] = 't';
    header[15] = ' ';
    header[16] = 16; /* 4 bytes: size of 'fmt ' chunk, Length of format data.  Always 16 */
    header[17] = 0;
    header[18] = 0;
    header[19] = 0;
    header[20] = 1; /* format = 1 ,Wave type PCM */
    header[21] = 0;
    header[22] = channels; /* channels */
    header[23] = 0;
    header[24] = (sampleRate & 0xff);
    header[25] = ((sampleRate >> 8U) & 0xff);
    header[26] = ((sampleRate >> 16U) & 0xff);
    header[27] = ((sampleRate >> 24U) & 0xff);
    header[28] = (byteRate & 0xff);
    header[29] = ((byteRate >> 8U) & 0xff);
    header[30] = ((byteRate >> 16U) & 0xff);
    header[31] = ((byteRate >> 24U) & 0xff);
    header[32] = (channels * bitsPerFrame / 8); /* block align */
    header[33] = 0;
    header[34] = bitsPerFrame; /* bits per sample */
    header[35] = 0;
    header[36] = 'd'; /*"data" marker */
    header[37] = 'a';
    header[38] = 't';
    header[39] = 'a';
    header[40] = (audioDataLen & 0xff); /* data-size (equals file-size - 44).*/
    header[41] = ((audioDataLen >> 8) & 0xff);
    header[42] = ((audioDataLen >> 16) & 0xff);
    header[43] = ((audioDataLen >> 24) & 0xff);
}

