...
*高速SD卡块写入测试函数
*/
void HD_write_text(sd_card_t* sd_struct)
{
  ...
  {
    SD_WriteBlocks(card, g_dataWrite, DATA_BLOCK_START+(counter*5), DATA_BLOCK_COUNT);
  }
...
void HD_read_text(sd_card_t* sd_struct)
{
  sd_card_t *card = sd_struct;
  int counter = 0;
  memset(g_dataWrite, 0x5aU, sizeof(g_dataWrite));
  ...
  /*开启定时器*/
  GPT2->CR |= GPT_CR_EN_MASK;
  PRINTF("第二次开启定时器初始值0：%d \r\n",GPT2->CNT);
  /*循环写入500个数据块*/
  for(counter=0; counter<4000; counter++)
  {
//    SD_WriteBlocks(card, g_dataWrite, DATA_BLOCK_START+(counter*5), DATA_BLOCK_COUNT);
    SD_ReadBlocks(card, g_dataRead, DATA_BLOCK_START+(counter*5), DATA_BLOCK_COUNT);
  }
