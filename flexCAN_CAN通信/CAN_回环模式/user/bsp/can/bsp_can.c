...
static void CAN_Mode_Config(void)
{
  ...
  */
  ...
  flexcanConfig.enableLoopBack = true;//设置为回环模式
...
void CAN_RX_Buffer_Config(void)
{
  ...
  mbConfig.id = FLEXCAN_ID_STD(0x123);           //消息缓冲区帧标识符，
  
  FLEXCAN_SetRxMbConfig(EXAMPLE_CAN, RX_MESSAGE_BUFFER_NUM, &mbConfig, true);//配置接受消息缓冲区
...
{
  FLEXCAN_SetTxMbConfig(EXAMPLE_CAN, TX_MESSAGE_BUFFER_NUM, true);//设置Tx消息缓冲区
  
  /* Enable Rx Message Buffer interrupt. */
  FLEXCAN_EnableMbInterrupts(EXAMPLE_CAN, 1 << RX_MESSAGE_BUFFER_NUM);//启用Rx消息缓冲区中断
  ...
  txFrame.id = FLEXCAN_ID_STD(0x123);           //设置帧ID （ID决定数据包在CAN网络上的发送优先级，以及有谁接收该帧）
