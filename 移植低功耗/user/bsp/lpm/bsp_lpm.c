...
void LPM_Init(void)
{
    ...
    CCM->CGPR |= CCM_CGPR_INT_MEM_CLK_LPM_MASK;//低功耗模式下不关闭 ARM Platform memories时钟

    /* Enable RC OSC. It needs at least 4ms to be stable, so self tuning need to be enabled. */
    XTALOSC24M->LOWPWR_CTRL |= XTALOSC24M_LOWPWR_CTRL_RC_OSC_EN_MASK;//切换到32.768K时钟
    /* Configure RC OSC */
    ...
    /* ERR007265 */
    IOMUXC_GPR->GPR1 |= IOMUXC_GPR_GPR1_GINT_MASK;//开启中断

    /* Initialize GPC to mask all IRQs */
    for (i = 0; i < LPM_GPC_IMR_NUM; i++)//屏蔽中断
    {
